...
table1 <- data.frame(theta = tau, gTheta = 100 * dchisq(tau, df = 10),
                     Mean = 100 * mean, StdDev = 100 * sd, Bias = 100* bias, CoefVar = sd / mean)
```
...
r2 <- deconv(tau = d$mu, X = data$z, deltaAt = 0, family = "Normal", pDegree = 5)

d$gMu <- approx(x = result$stats[, "theta"], y = result$stats[, "g"], xout = d$mu)$y

r <- t(sapply(tau, function(z) {
    gD <- dnorm(z, mean = d$mu) * dnorm(x = d$mu, mean = z)
    cumsum(gD) / sum(gD)
}))[, -c(1:2)]

findQuantile <- function(G) {
    approx(x = G,
           y = tau,
           xout = c(0.025, y = 0.975))$y
}

gQuantiles <- apply(r, 2, findQuantile)

d025 <- data.frame(z = d$z[-(1:2)], mu = gQuantiles[1, ])
d025 <- d025[order(d025$z), ]
d975 <- data.frame(z = d$z[-(1:2)], mu = gQuantiles[2, ])
d975 <- d975[order(d975$z), ]

plot(d025$z, d025$mu, type="l")
plot(d975$z, d975$mu, type="l")
...
We take $\mathcal{T} = (0.01, 0.02,\ldots, 0.09)$, so $m = 99.$  We take $Q$
