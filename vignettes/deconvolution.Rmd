---
title: "Empirical Bayes Deconvolution (Draft)"
...
The default setting for `deconv` uses the `Poisson` family---the only
one implemented currently---and a natural cubic spline basis of
degree 5 as $Q$. The regularization parameter for this example (`c0`)
is set to 1.
...
construct a table of values for various values of $\theta$.

```{r}
g <- sapply(results, function(x) x$stats[, "g"])
mean <- apply(g, 1, mean)
sd <- apply(g, 1, sd)
table1 <- data.frame(theta = tau, gTheta = 100 * dchisq(tau, df = 10),
...
plots <- lapply(c(1, 2, 4, 8), function(i) {
    ggplot() +
        ...
        labs(x = expression(theta), y = expression(g(theta)), title = sprintf("x = %d", i))
})
...
doublingTime <- uniroot(f = function(x) Rfn(x) - 1, interval = c(2, 4))$root
```
...
Below is a histogram of the data and that of the thetas.
...
Now we deconvolve this using $\tau = (-6, -5.75,\ldots, 3)$ and a
...
Per the above table, the estimated probability of $\mu = 0$ is `r round(result$stats[25, "g"], 3)`
$\pm$ `r round(result$stats[25, "SE.g"] , 3)`  with a bias of
about `r round(result$stats[25, "Bias.g"] , 3)`.

We can now plot the $g$-estimate removing the atom at 0.

```{r}
gData <- as.data.frame(result$stats[-25, c("theta", "g")])
...
4. Compute the estimated posterior density for of $\mu$ given $z$.

```{r, eval = FALSE}
w <- data[order(data$z), ]
zGrid <- w$z
gD <- result$stats$mat
mus <- sort(c(0, data$mu[9001:10000]))
closestG <- approx(x = gD[, "theta"], y = gD[, "g"], xout = mus)$y
gHat <- sapply(seq_len(length(mus)), function(i) dnorm(tau, mean = mus[i]) * closestG[i])
gHat <- apply(gHat, 2, function(x) cumsum(x) / sum(x))

gGivenZCDF <- function(x, z, g, theta) {
    sum(g * pnorm(x, mean =  z - theta))
}
gGivenZQuantile <- function(p, z) {
    uniroot(f = function(x) { gGivenZCDF(x, z, g = gDist[, "g"], theta = gDist[, "theta"]) - p },
            interval = c(-10, 10))$root
}

gQuantiles <- t(sapply(zGrid,
                       function(x) c(gGivenZQuantile(p = 0.025, z = x),
                                     gGivenZQuantile(p = 0.975, z = x))
                       ))
...
##  geom_line(mapping = aes(x = tau, y = gQuantiles[, 1]), color = "brown") +
##  geom_line(mapping = aes(x = zGrid, y = gQuantiles[, 2]), color = "brown") +
...
```{r}
p1 <- ggplot(mapping = aes(x = normEx2$theta)) +
    geom_histogram(mapping = aes(y  = ..count.. / sum(..count..) ),
                   color = "brown", bins = 60, alpha = 0.5) +
    labs(x = expression(theta), y = "Density")
set.seed (2332)
z <- rnorm(n = length(normEx2$theta), mean = normEx2$theta)
...
plots2 <- lapply(2:7,
                 function(p) {
                     result <- deconv(tau = tau, x = z, family = "Normal", pDegree = p)
                     g <- result$stats[, "g"]
                     ggplot(mapping = aes(x = normEx2$theta)) +
                         geom_histogram(mapping = aes(y = ..count.. / sum(..count..)),
                                        color = "brown", bins = 60, alpha = 0.2) +
                         geom_line(mapping = aes(x = tau, y = g), color = "blue") +
...
plots1 <- lapply(c(0.5, 1, 2, 4, 16, 32),
                 function(c0) {
                     result <- deconv(tau = tau, x = z, family = "Normal", pDegree = 6, c0 = c0)
                     g <- result$stats[, "g"]
                     ggplot(mapping = aes(x = z)) +
                         geom_histogram(mapping = aes(y = ..count.. / sum(..count..)),
                                        color = "brown", bins = 60, alpha = 0.2) +
...
We take $\tau = (0.01, 0.02,\ldots, 0.09)$, so $m = 99.$  We take $Q$
to be a 5-degree natural spline with columns standardized to mean 0 and
sum of squares equal to 1. The penalization parameter is set to 1. The
figure below shows the estimated prior density of $g(\theta)$.
...
```{r}
knitr::kable(d[indices, ])
```

### Bootstrap Comparison

As a check on the estimates of standard error and bias provided by
\code{deconv}, we compare the results with what we obtain using a
parametric boostrap.
...
bootResults <- lapply(seq_len(B),
                      function(x) {
                          ...
                          mat <- deconv(tau = tau, x = surgBoot, family = "Binomial", c0 = 1)$stats
                          mat[, c("g", "Bias.g")]
                      })
...
table2 <- data.frame(theta = tau,
                     ...
                     sdFormula = round(sqrt(diag(result$cov.g)) * 100, 3),
